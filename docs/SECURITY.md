...
| `PUBLISH_PUSH_TOKEN` secret (only if protection is ON) | Protection also blocks the post-merge bot's catalog-refresh push (`GH006` → auto-filed `CI-001`); the Actions app can't bypass rulesets on personal repos. Fine-grained PAT, **this repo only, Contents: Read+Write**, set as a repo secret — the post-merge workflow uses it when present, default bot token otherwise | GitHub → Settings → Developer settings → Fine-grained tokens; then `gh secret set PUBLISH_PUSH_TOKEN` |
