...
4. **Branch protection + repo hardening**: `gh api repos/<org>/<repo>/branches/main/protection` — required checks + code-owner review on? If 404: not protected — point at `/setup` step 6. If 403 "Upgrade to GitHub Pro": ⚠️ plan wall (private repo on the Free plan) — protection can't be enforced; note the compensating controls (CI gate on every PR) and recommend a paid org. Also check `gh api repos/<org>/<repo>/actions/permissions` — `allowed_actions: selected` expected, `sha_pinning_required: false` (deliberately — see `/setup` step 6).
