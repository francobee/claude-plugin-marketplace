...
### Changed
- **marketplace-admin 1.1.0**: `/setup` gains Quick mode (4 questions, safe defaults, full plain-English summary before writing) vs Advanced mode (every key with its one-line description); self-healing `gh` preflight; hosting-aware site step with graceful Pages→Cloudflare fallback; `/status` site check follows `site.hosting`.
