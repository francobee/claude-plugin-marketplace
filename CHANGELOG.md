...
- **Repo hardening baseline**: `.github/dependabot.yml` (weekly grouped PRs refreshing the SHA-pinned actions), a "Repo hardening" section in `docs/SECURITY.md` (protection, SHA-pinning enforcement, actions allowlist, plan-wall honesty), and the wizard now applies these settings and degrades gracefully when GitHub's Free plan refuses branch protection on private repos.
