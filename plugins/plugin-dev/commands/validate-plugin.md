...
5. `python3 scripts/permission_manifest.py --markdown plugins/<plugin-name>` — shows the reviewer's view: every MCP server, tool, hook, shell command, and endpoint the plugin touches. Walk the user through it; anything surprising here will also surprise the admin.
6. Optional but recommended before submitting: `python3 scripts/llm_review.py plugins/<plugin-name>` — the same Claude security review CI will run (prompt injection, exfiltration, hidden instructions). Needs `ANTHROPIC_API_KEY`.
